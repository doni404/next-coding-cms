...
// ** Actions Imports
import { fetchData, deleteGuideImage, updateGuideImage } from "src/store/apps/guide_image";
...
const RowOptions = ({ id, guideId, imgSrc }) => {
  // ** Hooks
  ...
  const handleEditClickOpen = async () => {
    ...
    const response = await axios.get(BASE_URL_API + 'v1/guides_image/' + id,
      header)
    setGuideImageId(response.data.data.id)
  ...
  const handleEditSubmit = () => {
    ...
    dispatch(updateGuideImage(request)).then((response) => {
      setOpenEdit(false);
      if (response.error) {
  ...
  const handleDelete = () => {
    ...
    dispatch(deleteGuideImage(data))
      .then((response) => {
  ...
  const imageWithAuth = async (image) => {
    ...
    const response = await axios.get(BASE_URL_API + 'v1/resources?type=guide_images&filename=' + image,
      header)
  ...
  return (
    <>
      <Box sx={{ display: "flex", alignItems: "center" }}>
        <Tooltip title="コピー">
          <IconButton size="small" onClick={() => {navigator.clipboard.writeText(BASE_URL_API + 'v1/public/resources?type=guide_images&filename='+imgSrc); toast.success("リンクがコーピしました。");}}>
            <ContentCopy />
          </IconButton>
        </Tooltip>
...
const columns = [
  ...
  },
  {
    ...
    headerAlign: "center",
    align: "center",
    renderCell: ({ row }) => <RowOptions id={row.id} guideId={row.guide.id} imgSrc={row.image} />,
  },
];

const GuideImageListTable = ({ guideId }) => {
  // ** State
  const [pageSize, setPageSize] = useState(7);

  const dispatch = useDispatch();
  const router = useRouter();
  const store = useSelector((state) => state.guide_image);
...
export default GuideImageListTable;
