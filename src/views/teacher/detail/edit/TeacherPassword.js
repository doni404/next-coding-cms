...
const schema = yup.object().shape({
  newPassword: yup.string().min(8, "パスワードは8文字以上です。").required(),
...
const TeacherPassword = ({data}) => {
  // ** Hooks
  const dispatch = useDispatch();
  const datares = data.data
  // states
  const [newPassword, setPassword] = useState('');
  const [confirmNewPassword, setConPassword] = useState('');
  const id = data.id;
  ...
  const handleMouseDownConfirmNewPassword = (event) => {
    ...
    axiosInstance.patch(BASE_URL_API + "v1/cms/teachers/"+ datares.id +"/update-password", request)
