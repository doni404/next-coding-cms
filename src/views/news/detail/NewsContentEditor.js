...
import { fetchData, deleteNewsMedia } from "src/store/apps/news_media";
import { updateContentNews } from "src/store/apps/news";

export const BASE_URL_API = process.env.REACT_APP_BASE_URL_API;

const NewsContent = ({setRefreshKey, data, id }) => {
  ...
  const onSubmit = () => {
    ...
    dispatch(fetchData(id))
      ...
      .then((originalPromiseResult) => {
        ...
        mediaNameDelete.forEach((item) => {
          const data = { news_id: id, id: item.id };
          console.log("data", data);

          dispatch(deleteNewsMedia(data))
            .unwrap()
            .then((response) => {
              if (response.error) {
    ...
    dispatch(updateContentNews({ id, dataContent }))
      .unwrap()
      .then((originalPromiseResult) => {
  ...
  class UploadAdapter {
    ...
    async upload() {
        ...
        try {
            const response = await axiosInstance.post(`${BASE_URL_API}v1/cms/news-media`,formData);
            if (response.data.data.file_name) {
                const fileURL = `${BASE_URL_API}v1/public/resources/news-media/${response.data.data.file_name}`;
