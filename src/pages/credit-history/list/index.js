...
// ** Actions Imports
import { fetchData, deleteGuide } from "src/store/apps/guide";

// ** Custom Components Imports
import TableHeader from "src/views/credit-history/list/TableHeader";
import SidebarAddGuide from "src/views/credit-history/list/AddGuide";
...
const RowOptions = ({ id, guideId }) => {
  ...
  return (
    <>
      ...
      >
        <MenuItem sx={{ p: 0 }}>
          <Link href={`/guide/general/detail/${id}`} passHref>
            <MenuItemLink>
...
const columns = [
  ...
  },
  {
    ...
    renderCell: ({ row }) => {
      return (
        <Typography noWrap variant="body2">
          {row.guide_category ? row.guide_category.name : "-"}
        </Typography>
      );
    },
  },
  ...
  },
  {
    ...
    headerAlign: "center",
    align: "center",
    renderCell: ({ row }) => <RowOptions id={row.id} guideId={row.guide_id} />,
  },
];
...
const GuideList = (props) => {
  ...
  // ** Hooks
  const dispatch = useDispatch();
  const store = useSelector((state) => state.guide);
  ...
  const toggleAddGuide = () => setAddGuideOpen(!addGuideOpen);
  ...
  return (
    <Grid container spacing={6}>
      ...
      <Grid item xs={12}>
        <Card>
          <Box sx={{ p: 5, pb: 1 }}>
            <Typography variant="h5">クレジット増減一覧</Typography>
          </Box>
          <TableHeader
            value={value}
            handleFilter={handleFilter}
            toggle={toggleAddGuide}
          />
          <DataGrid
      ...
      <SidebarAddGuide open={addGuideOpen} toggle={toggleAddGuide} />
    </Grid>
  );
};
